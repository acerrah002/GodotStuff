...
@export var lobby_scene: String = "res://UIScenes/lobby.tscn"
...
func _build_ui() -> void:
	...
	if OS.get_name() == "Android":
		upnp_status.text = "Status: Idle (UPnP rarely works on mobile networks - try Wi‑Fi (LAN) instead)"
	...
	# ----- Bluetooth Tab -----
	# Godot has no built-in Bluetooth API on macOS, Linux, or Android, so this
	# is not actually functional yet on any platform. Disabled rather than
	# left to silently "succeed" and then fail to connect.
	...
	bt_host_btn.disabled = true
	bt_join_btn.disabled = true
	bt_status.text = "Status: Not available yet (needs a platform Bluetooth plugin)"
...
func _connect_signals() -> void:
	...
	LanManager.lan_discovery_failed.connect(_on_lan_discovery_failed)
...
func _on_upnp_discovered(success: bool) -> void:
	if not success:
		upnp_status.text = "Status: UPNP discovery failed (common on mobile/some routers)."
		return
	upnp_status.text = "Status: Opening port %d..." % default_port
	var ok = Upnp.open_port(default_port, "CTF Game")
	if ok:
		NetworkManager.host_game(default_port, NetworkManager.local_player_name, true)
	else:
...
func _on_lan_discovery_failed() -> void:
	lan_status.text = "Status: No LAN game found (check you're on the same Wi‑Fi)."
