...
var team_mode_option: OptionButton
...
func _build_ui() -> void:
	...
	# Host team mode controls
	if is_host:
		var mode_row = _hbox()
		mode_row.add_child(_label("Team:", 16))
		team_mode_option = OptionButton.new()
		team_mode_option.add_item("Selective", 0)
		team_mode_option.add_item("Random", 1)
		team_mode_option.selected = NetworkManager.team_mode
		team_mode_option.item_selected.connect(_on_team_mode_changed)
		mode_row.add_child(team_mode_option)

		shuffle_btn = _btn("Shuffle", 14, 100, 35)
		shuffle_btn.pressed.connect(_on_shuffle_pressed)
		shuffle_btn.visible = NetworkManager.team_mode == 1
		mode_row.add_child(shuffle_btn)
...
# ---------- Player list & click handler ----------
func _refresh_player_list() -> void:
	player_list.clear()
	player_ids.clear()
	for id in NetworkManager.players:
		var p = NetworkManager.players[id]
		var team_str = "RED" if p.team == 0 else ("BLUE" if p.team == 1 else "None")
		var ping = NetworkManager.get_player_ping(id)
		var entry = "%s (%s) - %dms" % [p.name, team_str, ping]
		if id == 1:
			entry += " (Host)"
...
func _on_player_list_item_selected(index: int) -> void:
	...
	var selected_id = player_ids[index]
	var my_id = multiplayer.get_unique_id()

	if is_host and NetworkManager.team_mode == 0:
		var current_team = NetworkManager.players[selected_id].team
		var new_team = 0 if current_team != 0 else 1
		NetworkManager._force_team(selected_id, new_team)
	elif selected_id == my_id and NetworkManager.team_mode == 0:
		var current_team = NetworkManager.players[my_id].team
		var new_team = 0 if current_team != 0 else 1
		rpc_id(1, "_request_team", new_team)

func _on_team_changed(_id: int, _team: int) -> void:
	_refresh_player_list()

# ---------- Shuffle (host only) ----------
func _on_shuffle_pressed() -> void:
	if not is_host: return
	# Call directly (host is authority) – it will broadcast to clients
	NetworkManager._shuffle_teams()
...
func _update_team_mode_ui() -> void:
	if team_mode_option:
		team_mode_option.selected = NetworkManager.team_mode
	if shuffle_btn:
		shuffle_btn.visible = NetworkManager.team_mode == 1

func _on_team_mode_changed(idx: int) -> void:
	if not is_host: return
	var mode = idx
	rpc_id(1, "_set_team_mode", mode)
	NetworkManager.team_mode = mode
...
func _on_start_game() -> void:
	...
	print("[Lobby] Calling NetworkManager.start_game_countdown()")
...
func _on_countdown_update(seconds: int) -> void:
	...
	if seconds == 0:
		...
		start_game_btn.disabled = true
		# Load the game scene (everyone does this)
...
func _on_leave() -> void:
	...
	get_tree().change_scene_to_file("res://Scenes/UIScenes/MainMenu.tscn")
